...
sankey <- function(thedata, col="white", showLabels=TRUE,title) {
    ...
    # Get your Sankey on.
    plot(0, 0, xlim=c(0, maxdepth+1), ylim=c(-startTotal/10, startTotal), type="n", ylab="value",main=paste0(title),xaxt='n',xlab="")
    for (i in 1:maxdepth) {
    	...
    	for (j in 1:length(leaves[,1])) {
    		...
    		if (leaves[j,]$parent == '') {
    			...
    			rect(x1, y1, x2, y2, col=col, border="black")
                lines(c(x1, x2), c(y2, y2), col="black", lwd=0.7)
    			...
    			rect(x1, y1, x2+0.01, y2, col=col, border="black")
    			lines(c(x1, x2), c(y2, y2), col="black", lwd=0.7)
    			...
    			if (leaves[j,]$name %in% thedata$parent) {
    				...
    				polygon(c(x1, x2, x3, x4), c(y1, y2, y3, y4), col=col, border="black")
    				lines(c(x1, x2), c(y1, y2), col="black", lwd=0.7)
